...
public:
    ...
    /** How far below the character to trace for a slide floor normal (UU). */
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Crux|Slide")
    float SlideFloorTraceLength = 120.f;

    /** Speed cap reported while sliding. */
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Crux|Slide")
    ...
    /** Maximum upward floor-normal dot allowed for wall-run surfaces. */
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Crux|WallRun")
    float WallRunMaxFloorAngleDot = 0.3f;

    /** Speed cap reported while wall-running. */
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Crux|WallRun")
    float MaxWallRunSpeed = 1600.f;
    ...
    /** Portion of downward speed converted into forward glide speed each tick. */
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Crux|Wingsuit")
    float WingsuitDiveTransferRate = 0.35f;
    ...
    // -------------------------------------------------------------------------
    // READABLE STATE — queried by the Character for input routing
    // -------------------------------------------------------------------------

    /** True while the player is in CMOVE_Slide. Read by the Character for SlideHop. */
    UPROPERTY(BlueprintReadOnly, Category = "Crux|State")
    bool bIsSliding = false;

    /** True while the player is in CMOVE_WallRun. Read by the Character for WallJump. */
    UPROPERTY(BlueprintReadOnly, Category = "Crux|State")
    bool bIsWallRunning = false;

    /** True while wingsuit is active. */
    UPROPERTY(BlueprintReadOnly, Category = "Crux|State")
...
protected:
    ...
    float   AirControlBeforeMicroCharge = 0.6f;      // Restored after held charge lock

    FTimerHandle MicroChargeTimerHandle;

    /** How long held micro-charge locks air steering. */
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Crux|MicroCharge")
    float MicroChargeAirControlLockDuration = 0.2f;
...
private:
    ...
    /** Restore air control after a held micro-charge lock expires. */
...
/** Saved-move state for client prediction of Crux custom movement flags. */
...
private:
    ...
    uint8 SavedCustomMovementMode = CMOVE_MAX;
};

/** Client prediction data factory that allocates Crux saved moves. */
class FNetworkPredictionData_Client_Crux : public FNetworkPredictionData_Client_Character
{
public:
    explicit FNetworkPredictionData_Client_Crux(const UCharacterMovementComponent& ClientMovement);
